...
const impactMetrics = [
  {
    ...
    value: "12k+",
    description: "Weekly letters, audio meditations, and journal prompts offered to our community.",
  },
  {
    label: "Members in Circle",
    value: "5k",
    description: "BIPOC femmes, queer creatives, and co-conspirators healing together across time zones.",
  },
  {
    label: "Accessibility Audits",
    value: "90%",
    description: "Interfaces that pass manual review for contrast, motion, and screen-reader flow.",
  },
]
...
const missionPillars = [
  {
    title: "Mission",
    copy:
      "Fuse creativity, technology, and recovery into sustainable income streams and gentle autonomy for every member who enters our sanctuary.",
  },
  {
    title: "Vision",
    copy:
      "A rooted network of healing artists who earn with ease, share resilient stories, and extend rest-forward tools throughout the South and beyond.",
  },
  {
    title: "Values",
    copy:
      "Soft power, ritual, transparency, cultural honor, accessibility, interdependence, and becoming guide every interface we craft.",
  },
]
        ...
        <div className="relative container mx-auto px-6 flex flex-col gap-12 lg:flex-row lg:items-center">
          <div className="max-w-2xl space-y-6">
            ...
            <p className="font-lora text-lg leading-relaxed text-magnolia-white/80">
              Midnight Magnolia is a Rumi-Nations LLC studio founded by Latisha Vincent-Waters. We merge poetic storytelling, trauma-informed technology, and ancestral care so recovering creatives can build income with gentleness. Rest is strategy. Creation is power.
            </p>
            <p className="font-lora text-lg leading-relaxed text-magnolia-white/80">
              Every product, workshop, and automation is crafted to lighten the load for BIPOC femmes, queer kin, and neurodivergent folks navigating sobriety, chronic illness, or transition. We design digital sanctuaries that feel like a handwritten letter under moonlight.
            </p>
          </div>
          <div className="grid w-full max-w-xl grid-cols-1 gap-6 sm:grid-cols-3 lg:grid-cols-1">
            {impactMetrics.map((metric) => (
              ...
              >
                <p className="font-playfair text-3xl font-semibold text-gold">{metric.value}</p>
                <p className="font-montserrat text-sm uppercase tracking-wide text-magnolia-white/70">
                  {metric.label}
                </p>
                <p className="mt-3 font-lora text-sm leading-relaxed text-magnolia-white/70">{metric.description}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="border-y border-magnolia-white/10 bg-[#0F223C] py-20">
        <div className="container mx-auto grid gap-10 px-6 lg:grid-cols-3">
          {missionPillars.map((pillar) => (
            <div key={pillar.title} className="flex flex-col gap-4 rounded-3xl border border-magnolia-white/10 bg-magnolia-white/5 p-8">
              <h2 className="font-playfair text-2xl text-gold">{pillar.title}</h2>
              <p className="font-lora text-base leading-relaxed text-magnolia-white/80">{pillar.copy}</p>
            </div>
          ))}
        </div>
      </section>
        ...
        <div className="container mx-auto flex flex-col gap-12 px-6 lg:flex-row lg:items-start">
          ...
          <div className="grid flex-1 grid-cols-1 gap-6 sm:grid-cols-2">
            {offerSuite.map((offer) => (
              <article key={offer.title} className="rounded-3xl border border-magnolia-white/10 bg-magnolia-white/5 p-6">
                <h3 className="font-playfair text-2xl text-gold">{offer.title}</h3>
                <p className="mt-4 font-lora text-base leading-relaxed text-magnolia-white/80">{offer.details}</p>
              </article>
            ))}
          </div>
        </div>
      ...
      <section className="border-t border-magnolia-white/10 bg-[#0F223C] py-20">
        <div className="container mx-auto px-6">
          ...
          <p className="mt-4 max-w-2xl font-lora text-lg leading-relaxed text-magnolia-white/80">
            Our roadmap honors seasonal pacing. Each phase deepens relationship, expands access, and keeps automation in service of human care.
          </p>
          <div className="mt-12 grid gap-8 lg:grid-cols-2">
            {timeline.map((entry) => (
              <div key={entry.phase} className="rounded-3xl border border-gold/20 bg-magnolia-white/5 p-8">
                <p className="font-montserrat text-xs uppercase tracking-[0.3em] text-gold/80">{entry.timeframe}</p>
                <h3 className="mt-3 font-playfair text-2xl text-gold">{entry.phase}</h3>
                <p className="mt-4 font-lora text-base leading-relaxed text-magnolia-white/80">{entry.description}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="container mx-auto px-6">
          <div className="grid gap-10 lg:grid-cols-2">
            <div className="space-y-6">
              <h2 className="font-playfair text-4xl">Accessibility & Care Rituals</h2>
              <p className="font-lora text-lg leading-relaxed text-magnolia-white/80">
            ...
            <div className="flex flex-col gap-6 rounded-3xl border border-magnolia-white/10 bg-magnolia-white/5 p-8">
              <h3 className="font-playfair text-2xl text-gold">Stewardship Practices</h3>
              <p className="font-lora text-base leading-relaxed text-magnolia-white/80">
                We rotate keys quarterly, encrypt sensitive data, and provide export-ready reflections upon request. Automations are documented in Notion and audited monthly alongside our Make.com scenarios.
              </p>
              <p className="font-lora text-base leading-relaxed text-magnolia-white/80">
                Operations run on gentle rhythms: Plausible analytics, Stripe reporting, and Wise multi-currency payouts keep finances transparent while fueling rest stipends for the community.
              </p>
              <p className="font-lora text-base leading-relaxed text-magnolia-white/80">
                When issues surface, we move them through our Notion Kanban ritual—Backlog → Ritualize → Ready → Blooming—so nothing sacred slips through the cracks.
              </p>
            </div>
          </div>
        </div>
      ...
      <section className="relative overflow-hidden border-t border-magnolia-white/10 bg-[#0F223C] py-20">
        ...
        <div className="relative container mx-auto flex flex-col items-center gap-6 px-6 text-center">
          ...
          <p className="max-w-2xl font-lora text-lg leading-relaxed text-magnolia-white/80">
            Step into a sanctuary that honors your nervous system and your ambition. Whether you crave a guided journal, an automated launch ritual, or a circle that remembers your name, Midnight Magnolia is ready to welcome you home.
          </p>
